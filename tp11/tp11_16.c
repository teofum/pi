...
int main(void) {
  assert(phrases_new(1200, 1100) == NULL);
  phrasesADT p = phrases_new(1200, 1800);
  assert(p != NULL);
  assert(phrases_put(p, 1199, "Hello") == 0);
  assert(phrases_size(p) == 0);
  assert(phrases_put(p, 1200, "Hello") == 1);
  assert(phrases_put(p, 1210, ", ") == 1);
  assert(phrases_put(p, 1220, "world") == 1);
  assert(phrases_size(p) == 3);

  char *aux = phrases_get(p, 1100);
  assert(aux == NULL);
  aux = phrases_get(p, 1200);
  assert(strcmp(aux, "Hello") == 0);
  free(aux);
  aux = phrases_get(p, 1210);
  assert(strcmp(aux, ", ") == 0);
  free(aux);
  aux = phrases_get(p, 1220);
  assert(strcmp(aux, "world") == 0);
  free(aux);

  aux = phrases_concat_all(p);
  assert(strcmp(aux, "Hello, world") == 0);
  free(aux);

  aux = phrases_concat(p, 1230, 1800);
  assert(strcmp(aux, "") == 0);
  free(aux);

  aux = phrases_concat(p, 1200, 1800);
  assert(strcmp(aux, "Hello, world") == 0);
  free(aux);

  // reemplazamos "world" por otro texto
  assert(phrases_put(p, 1220, "mundo cruel.") == 1);
  assert(phrases_size(p) == 3);
  aux = phrases_concat_all(p);
  ...
  char *aux1 = phrases_get(p, 1);

  char aux12[strlen(aux1) + strlen(aux) + 1];
  strcpy(aux12, aux1);
  strcat(aux12, aux);

  char *aux10 = phrases_get(p, 10);
